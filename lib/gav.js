var addDimension, addMetric, checkDimension, checkFilter, checkMetric, checkSegment, checkSort, dimensions, getDimension, getMetric, metrics;
...
checkDimension = function(dim) {
  return getDimension(dim) != null;
};

checkMetric = function(met) {
  return getMetric(met) != null;
};
...
checkFilter = function(filter) {
  var and_exp, and_exps, components, exp_type, or_exp, or_exps, _i, _j, _len, _len1;
  ...
  for (_i = 0, _len = and_exps.length; _i < _len; _i++) {
    ...
    for (_j = 0, _len1 = or_exps.length; _j < _len1; _j++) {
      or_exp = or_exps[_j];
      if (exp_type == null) {
        exp_type = new RegExp('(==|!=|>|<|>=|<=)[0-9]+').test(or_exp) ? 'metric' : 'dimension';
      }
